...
int charMapped(char validChar){ // sets each valid character to its corresponding index of the next array of a dictNode
    switch(validChar){
        case 'a':
        case 'A':
            return 0;
...
dictNode::dictNode() { // default constructor
    isWord = false;
    for(int i = 0; i < NUMOFCHARS; i++)
        next[i] = nullptr;
};

dictNode::dictNode(bool isWordVal){ // constructor with a parameter that allows for the isWord to be made true on creation
    isWord = isWordVal;
    for(int i = 0; i < NUMOFCHARS; i++)
        next[i] = nullptr;
};
